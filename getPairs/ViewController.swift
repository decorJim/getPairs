...
class ViewController: UIViewController {
   ...
   var baseurl: String = "https://serverimage.onrender.com/" //"http://localhost:8080/"
    ...
    func retrieveRecentPhotos(n: Int) {
        ...
        var i = 0
        while i < n {
            self.processNextPair(i: i, assets: assets)
            i += 2
        }
        DispatchQueue.main.async {
            let alert = UIAlertController(title: "Title", message: "all images processed !", preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .default)
            alert.addAction(action)
            self.present(alert, animated: true)
        }
    ...
    func processNextPair(i: Int, assets: PHFetchResult<PHAsset>) {
        if i+1 > assets.count {
            return
        }
        ...
        manager.requestImageData(for: asset1, options: options) { (data, _, _, _) in
    ...
    func processImages(firstImage:UIImage,secondImage:UIImage) {
            ...
            UIGraphicsEndImageContext()
             if let unwrappedSymmetricImage = symmetricImage {
                UIImageWriteToSavedPhotosAlbum(unwrappedSymmetricImage, nil, nil, nil)
             }
    ...
    func retrieveRecentPhotos(n: Int) {
        ...
        DispatchQueue.main.async {
            let alert = UIAlertController(title: "Title", message: "all images processed !"+String(i), preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .default)
            alert.addAction(action)
            self.present(alert, animated: true)
        }
